...
swpcarbontotal <- function(Yrs = 1990:2015, distribution = c("Exponential",
    "K=2"), onlytotal = TRUE, lumberpre = TRUE, approach = c("Production",
    ...
    for (year in Yrs) {
        ...
        for (eu in 1:13) {
            ...
            if (type == "K=2") {
                ...
                decays <- decayarray[2, eu, yearrange, year - minyr +
                  1]

            }
            Var2_totalC_SWP[Var2_totalC_SWP$Year == year, paste("EU",
                eu, sep = "")] <- sum(placeIU[yearrange, eu] * decays *
                (1 - lossIU[yearrange, 1]))
        }
    }

    Var2_totalC_SWP[, "LumberPre1900"] <- lumberpre1900[Yrs - minyr +
        1, ]
